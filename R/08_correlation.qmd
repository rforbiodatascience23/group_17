...
c_sorted$subclass <- factor(c_sorted$subclass, levels = unique(c_sorted$subclass))
c_sorted$status <- factor(c_sorted$status, levels = unique(c_sorted$status))
  ...
  theme_minimal() +
  coord_equal() +
  labs(title = "Correlation matrix of tumor subclasses and ER, PR and her2 status", x = "Subclass Label", y = "Status Label", fill = "Correlation") +
