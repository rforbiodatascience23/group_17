...
pca_plot <- pca_aug |> 
  ggplot(aes(x = .fittedPC1, # PC1 on x axis
             ...
             colour = pam50, # color based on pam50 
             fill = pam50)) + # fill based on pam50
  ...
  geom_point(shape = 21, # global are already given above and we not just apply design for the dots 
             size = 6,
             alpha = 0.5) + # transparency of the dots is set to 0.5
  labs(
    title = "PCA", # title of the data
...
PCA_kmeans <- pca_aug |>
  mutate(cluster = k_means_clusters)|> #Creating a new column called Cluster with results from the K_means clustering analysis. 
    ggplot(aes(x = .fittedPC1,
             y = .fittedPC2,
             colour = k_means_clusters, # Coloring by the clusters
             fill = k_means_clusters)) + # Filling by the clusters
  ...
  geom_point(shape = 21,
             ...
             alpha = 0.5) +
  labs(
    title = "PCA catagorized through clusters",
